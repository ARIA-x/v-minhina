...
client.on('message', (topic, message) => {
  ...
  if (topic.startsWith('/test')) {
    ...
    // for page 1
      ...
      console.log("bunbunhello")
      // console.log(parseInt(topic.split("/")[4]))
      ...
      try {
        buf = Buffer.from(JSON.stringify(routes[id]), 'utf-8')
        client.publish(retTopic, buf)
      } catch (err) {
        console.log(err)
        client.publish(retTopic, Buffer.from(("err"), 'utf-8'))
      }
  ...
  // if recv result from evac-sim, compute
    ...
    // console.log(topic.split("/")[4])

    route = []
    for (let i in a) {
      let x = nodesjson[a[i]][0];
      let y = nodesjson[a[i]][1];
    ...
    // console.log(routes)
    // console.log(util.inspect(routes, { maxArrayLength: null }));
    // console.log(r)

    // buf = Buffer.from(r, 'utf8');
    // client.publish("test/1/evacroute", Buffer.from(String(routes), 'utf-8') )
  } else if (topic.startsWith('aria/intra/persons/default')){
      infl = []
      for( const prop in a){
        // console.log(prop, a[prop])
        infl[a[prop]['ID']] = a[prop]['influence']
      }
      // console.log("infl", infl)
      // console.log(util.inspect(infl, { maxArrayLength: null }));


  } else if (topic.startsWith('/person/send/all')){
    console.log(topic,a[1])

    for( const prop in a){
      console.log(prop, a[prop])
      sta[a[prop]['id']] = a[prop]['status']
    }
    console.log("sta:", sta)
    console.log(util.inspect(sta, { maxArrayLength: null }));
